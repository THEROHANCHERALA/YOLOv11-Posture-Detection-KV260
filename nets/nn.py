...
class Residual(torch.nn.Module):
    def __init__(self, ch, e=0.5):
        super().__init__()
        self.conv1 = Conv(ch, int(ch * e), torch.nn.Hardsigmoid(), k=3, p=1)
        self.conv2 = Conv(int(ch * e), ch, torch.nn.Hardsigmoid(), k=3, p=1)
...
class CSPModule(torch.nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.conv1 = Conv(in_ch, out_ch // 2, torch.nn.Hardsigmoid())
        self.conv2 = Conv(in_ch, out_ch // 2, torch.nn.Hardsigmoid())
        self.conv3 = Conv(out_ch, out_ch, torch.nn.Hardsigmoid())   # fixed
...
class CSP(torch.nn.Module):
    def __init__(self, in_ch, out_ch, n, csp, r):
        super().__init__()
        self.conv1 = Conv(in_ch, 2 * (out_ch // r), torch.nn.Hardsigmoid())
        self.conv2 = Conv((2 + n) * (out_ch // r), out_ch, torch.nn.Hardsigmoid())
...
class SPP(torch.nn.Module):
    def __init__(self, in_ch, out_ch, k=5):
        super().__init__()
        self.conv1 = Conv(in_ch, in_ch // 2, torch.nn.Hardsigmoid())
        self.conv2 = Conv(in_ch * 2, out_ch, torch.nn.Hardsigmoid())
...
class Attention(torch.nn.Module):
    def __init__(self, ch, num_head):
        super().__init__()
        self.conv1 = Conv(ch, ch, torch.nn.Hardsigmoid(), k=3, p=1, g=ch)
        self.conv2 = Conv(ch, ch, torch.nn.Hardsigmoid())
...
class PSABlock(torch.nn.Module):
    def __init__(self, ch, num_head):
        ...
        self.conv2 = torch.nn.Sequential(
            Conv(ch, ch * 2, torch.nn.Hardsigmoid()),
...
class PSA(torch.nn.Module):
    def __init__(self, ch, n):
        super().__init__()
        self.conv1 = Conv(ch, 2 * (ch // 2), torch.nn.Hardsigmoid())
        self.conv2 = Conv(2 * (ch // 2), ch, torch.nn.Hardsigmoid())
...
class DarkNet(torch.nn.Module):
    def __init__(self, width, depth, csp):
        ...
        # p1/2
        self.p1.append(Conv(width[0], width[1], torch.nn.Hardsigmoid(), k=3, s=2, p=1))
        # p2/4
        self.p2.append(Conv(width[1], width[2], torch.nn.Hardsigmoid(), k=3, s=2, p=1))
        self.p2.append(CSP(width[2], width[3], depth[0], csp[0], r=4))
        # p3/8
        self.p3.append(Conv(width[3], width[3], torch.nn.Hardsigmoid(), k=3, s=2, p=1))
        self.p3.append(CSP(width[3], width[4], depth[1], csp[0], r=4))
        # p4/16
        self.p4.append(Conv(width[4], width[4], torch.nn.Hardsigmoid(), k=3, s=2, p=1))
        self.p4.append(CSP(width[4], width[4], depth[2], csp[1], r=2))
        # p5/32
        self.p5.append(Conv(width[4], width[5], torch.nn.Hardsigmoid(), k=3, s=2, p=1))
...
class DarkFPN(torch.nn.Module):
    def __init__(self, width, depth, csp):
        ...
        self.h3 = Conv(width[3], width[3], torch.nn.Hardsigmoid(), k=3, s=2, p=1)
        self.h4 = CSP(width[3] + width[4], width[4], depth[5], csp[0], r=2)
        self.h5 = Conv(width[4], width[4], torch.nn.Hardsigmoid(), k=3, s=2, p=1)
...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=()):
        ...
        self.box = torch.nn.ModuleList(torch.nn.Sequential(
            Conv(x, box, torch.nn.Hardsigmoid(), k=3, p=1),
            Conv(box, box, torch.nn.Hardsigmoid(), k=3, p=1),
            torch.nn.Conv2d(box, out_channels=4 * self.ch, kernel_size=1)
        ) for x in filters)
        self.cls = torch.nn.ModuleList(torch.nn.Sequential(
            Conv(x, x, torch.nn.Hardsigmoid(), k=3, p=1, g=x),
            Conv(x, cls, torch.nn.Hardsigmoid()),
            Conv(cls, cls, torch.nn.Hardsigmoid(), k=3, p=1, g=cls),
            Conv(cls, cls, torch.nn.Hardsigmoid()),
    ...
    def forward(self, x):
        ...
        return x  # raw multi-scale maps for DPU quantization
